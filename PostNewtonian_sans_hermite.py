...
def compute_accelerations_and_jerks(positions, velocities, masses, accelerations_pert_old, dt):
    ...
    for i in prange(n):
        ...
        for j in range(n):
            if i != j and masses[j] >= 10*h*nu/c**2:
                ...
                for k in range(n):
                    if k != i and k != j and masses[k] >= 10*h*nu/c**2:
                        # Relative vectors
...
accelerations_pred_old = np.zeros_like(accelerations_pert_old)
accelerations_old = np.zeros_like(accelerations_pert_old)
masses = np.random.rand(n_bodies) * 1e32      # in kg
